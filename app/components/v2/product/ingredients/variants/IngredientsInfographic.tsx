...
export function IngredientsInfographic({ data }: IngredientVariantProps) {
  ...
  const Row = ({ ing }: { ing: Ingredient }) => (
    <div className="flex justify-between items-baseline gap-2.5 py-2.5 border-b border-dashed border-[#e2e6ee] last:border-b-0">
    ...
    <div className="flex flex-col gap-5">
      ...
      >
        <div className="text-center mb-4">
          ...
          <p className="text-[13px] text-[#667085] mt-0.5">{labels.whatsInsideSub}</p>
        </div>
        ...
        {hero.map((ing, i) => (
          ...
          >
            <CategoryBadge category="immune" size={48} />
            <span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {probiotics.length > 0 && (
            <div>
              <h4 className="inline-flex items-center gap-1.5 text-white text-[12px] font-bold tracking-[0.1em] uppercase px-3.5 py-1.5 rounded-full mb-3 bg-[#5d6fcd]">
                <Dna className="w-3.5 h-3.5" /> {labels.categories.probiotic}
              </h4>
          ...
          {rightColumn.length > 0 && (
            <div>
              <h4 className="inline-flex items-center gap-1.5 text-white text-[12px] font-bold tracking-[0.1em] uppercase px-3.5 py-1.5 rounded-full mb-3 bg-[#d9a521]">
                <Sparkles className="w-3.5 h-3.5" /> {labels.categories.prebiotic} / {labels.categories.vitamin}
              </h4>
